...
class DatasetTool:
    ...
    def __init__(self, task_type: TaskType):
        ...
        self.mapper = hugging_face_dataset_mappers.get(task_type)
    ...
    def __init_datasets(self) -> dict[str, Dataset]:
        ...
        for dataset in all_datasets:
            if task_category in dataset.tags:
                if self.task_type in self.available_mappers_flag:
                    ...
                    specific_datasets[dataset.id] = Dataset(
    ...
    def __check_dataset(self, dataset: ObjectInfo, datasets_dict: dict) -> None:
        ...
        if (
            status is True
            and self.mapper.is_correct(info_dict, dataset.id) is True
        ):
    ...
    def download_dataset(self, name: str):
        ...
        if self.task_type in self.available_mappers_flag:
            return self.mapper.map(dataset_dict)
        return dataset_dict

    @staticmethod
    def create_dir(dataset_path: str):
