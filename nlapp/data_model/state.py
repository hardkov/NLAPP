from typing import Final


class KEYS:
    SELECTED_PAGE: Final = "selectedPage"
    SELECTED_MODEL: Final = "selectedModel"
    SELECTED_DATASET: Final = "selectedDataset"
    IS_MODEL_CACHED: Final = "isModelCached"
    IS_DATASET_CACHED: Final = "isDatasetCached"
    MODEL_LIST: Final = "modelList"
    DATASET_LIST: Final = "datasetList"
    SELECTED_TASK: Final = "selectedTask"
    MODEL_FETCHING_TOGGLED: Final = "modelFetchingToggled"
    INITIALIZATION_DONE: Final = "initializationDone"
